      ...
      >
        ...
        <div className="mb-6 flex gap-3">
          <button
            ...
            onClick={() => {
              ...
              if (!clientId) { infoToast("Google sign in not configured"); return; }
              const redirectUri = window.location.origin + "/oauth/callback";
              const state = Math.random().toString(36).substring(2);
              sessionStorage.setItem("oauth_state", state);
              sessionStorage.setItem("oauth_provider", "google");
              window.location.href = "https://accounts.google.com/o/oauth2/v2/auth?client_id=" + clientId + "&redirect_uri=" + encodeURIComponent(redirectUri) + "&response_type=code&scope=https://www.googleapis.com/auth/userinfo.email%20https://www.googleapis.com/auth/userinfo.profile&state=" + state;
            }}
            className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-border bg-background/50 px-4 py-3 text-xs font-semibold text-muted-foreground transition-all duration-200 hover:bg-background/80 hover:text-foreground active:scale-[0.97] disabled:opacity-50"
          >
          ...
          <button
            ...
            onClick={() => {
              ...
              if (!clientId) { infoToast("GitHub sign in not configured"); return; }
              const redirectUri = window.location.origin + "/oauth/callback";
              const state = Math.random().toString(36).substring(2);
              sessionStorage.setItem("oauth_state", state);
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div>
            ...
            <div
              className={`flex h-13 items-center gap-3 rounded-2xl border bg-background/70 px-4 transition-all duration-300 focus-within:shadow-glow ${
          ...
          <div className="flex items-center justify-between">
            <label className="flex cursor-pointer items-center gap-2">
              <input
                ...
                className="h-4 w-4 rounded border-border bg-background text-primary accent-primary focus:ring-primary/30"
              />
